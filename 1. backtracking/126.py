...
class Solution:
    def findLadders(self, beginWord, endWord, wordList):
        ...
        self.findShortestPath(beginWord, endWord, wordList)
        return self.backtracking(beginWord, endWord)
    ...
    def findShortestPath(self, beginWord, endWord, wordList):
        ...
        res = [[beginWord]]
        self.go_back = {w:[] for w in wordList}
        layer_count = 1
        # bfs
        while current_layer and not_find_end_word:

            print(layer_count,current_layer)
                ...
                for next_word in wordList:
                    for k in range(self.word_length):
                        if word[k] != next_word[k]:
                            break
                    if word[k+1:] == next_word[k+1:]:

                        if next_word == endWord:
                            will_visited.add(next_word)
                            next_layer.add(next_word)
                            not_find_end_word = False
                            self.go_back[endWord].append(word)

                        elif next_word not in visited:
                            will_visited.add(next_word)
                            next_layer.add(next_word)
                            self.go_back[next_word].append(word)

                        else:
                            pass

            for i in will_visited:
                visited.add(i)
            will_visited.clear()
            layer_count += 1
    ...
    def backtracking(self, beginWord, endWord):
        res = []
        ...
        def deeper(path):
            ...
            if path[-1] in self.go_back:
                for i in self.go_back[path[-1]]:
                    deeper(path+[i])
