...
class Solution:
    def widthOfBinaryTree(self, root):
        ...
        while current_layer:
            width = 0
            # print(current_layer, current_count)
    ...
    def widthOfBinaryTree1(self, root):
        ...
        none_count = 0
        while node_count > 0:
            # print(q, node_count, width)
            node = q.popleft()
            if node is split:
                max_width, width = max(max_width, width), 0
                q.append(split)
                none_count = 0
            elif node is None and width > 0:
                q.extend([None, None])
                none_count += 1
            elif node:
                ...
                if node.right:
                    node_count += 1
            else:# leading None node is a layer
                pass
